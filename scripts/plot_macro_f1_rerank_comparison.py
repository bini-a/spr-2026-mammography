#!/usr/bin/env python3
"""
Render a compact SVG comparing specialist reranking versus no reranking.

Two direct A/B pairs are shown:
1. exp025_multiseed_ensemble -> exp034_rerank_a023_0_4_a456_0_2
2. exp998_blend_base         -> exp999_rerank_a023_0_4_a456_0_2

This captures both the main honest win and a later blended-base rerank that did
not improve, which makes the comparison more informative than a single example.
"""
...
COMPARISONS = [
    Comparison(
        title="Rerank on multiseed ensemble",
        reference_label="No reranking (exp025)",
        reference_experiment="exp025_multiseed_ensemble",
        variant_label="Specialist reranking (exp034)",
        variant_experiment="exp034_rerank_a023_0_4_a456_0_2",
    ),
    Comparison(
        title="Rerank on blended base",
        reference_label="No reranking (exp998)",
        reference_experiment="exp998_blend_base",
        variant_label="Specialist reranking (exp999)",
        variant_experiment="exp999_rerank_a023_0_4_a456_0_2",
    ),
]
...
def render_svg(repo_root: Path) -> str:
    ...
    parts = [
        ...
        '<title id="title">Performance comparison: specialist reranking</title>',
    ...
    parts.append(
        svg_text(
            72,
            90,
            "Performance Comparison: Specialist Re-ranking",
            fill=INK,
    ...
    footnote = (
        ...
        "Pairs: exp025 vs exp034, exp998 vs exp999"
    )
    parts.append(
        svg_text(
            72,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="notebooks/figures/macro_f1_rerank_comparison.svg",
