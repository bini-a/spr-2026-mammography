#!/usr/bin/env python3
"""
Render a compact SVG comparing synthetic-data augmentation against no synthetic data.

The clean reference for both comparisons is exp023_bertimbau_dedup, since the
synthetic runs reuse the same honest group-aware + dedup pipeline and change
only the synthetic augmentation setup.
"""
...
COMPARISONS = [
    Comparison(
        title="Broad minority-class augmentation",
        reference_label="No synthetic data (exp023)",
        reference_experiment="exp023_bertimbau_dedup",
        variant_label="Synthetic {0,3,4,5,6} (exp030)",
        variant_experiment="exp030_bertimbau_synthetic_rare",
    ),
    Comparison(
        title="Targeted ultra-rare augmentation",
        reference_label="No synthetic data (exp023)",
        reference_experiment="exp023_bertimbau_dedup",
        variant_label="Synthetic c5/c6 only (exp031)",
        variant_experiment="exp031_bertimbau_synthetic_c56",
    ),
]
...
def render_svg(repo_root: Path) -> str:
    ...
    parts = [
        ...
        '<title id="title">Performance comparison: synthetic data augmentation</title>',
    ...
    parts.append(
        svg_text(
            72,
            90,
            "Performance Comparison: Synthetic Data Augmentation",
            fill=INK,
    ...
    footnote = (
        ...
        "Pairs: exp023 vs exp030, exp023 vs exp031"
    )
    parts.append(
        svg_text(
            72,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="notebooks/figures/macro_f1_synthetic_comparison.svg",
