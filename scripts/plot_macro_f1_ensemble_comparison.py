#!/usr/bin/env python3
"""
Render a compact SVG comparing ensembling versus no ensembling.

Two representative A/B pairs are shown:
1. exp010_bertimbau_es -> exp014_ensemble_v1
2. exp023_bertimbau_dedup -> exp025_multiseed_ensemble

This captures both the earlier leaky-fold ensemble uplift and the later honest
multiseed ensemble uplift.
"""
...
COMPARISONS = [
    Comparison(
        title="Leaky-fold OOF ensemble",
        reference_label="No ensembling (exp010)",
        reference_experiment="exp010_bertimbau_es",
        variant_label="Post-hoc ensemble (exp014)",
        variant_experiment="exp014_ensemble_v1",
    ),
    Comparison(
        title="Honest multiseed ensemble",
        reference_label="No ensembling (exp023)",
        reference_experiment="exp023_bertimbau_dedup",
        variant_label="4-seed ensemble (exp025)",
        variant_experiment="exp025_multiseed_ensemble",
    ),
]
...
def render_svg(repo_root: Path) -> str:
    ...
    parts = [
        ...
        '<title id="title">Performance comparison: ensembling</title>',
    ...
    parts.append(
        svg_text(
            72,
            90,
            "Performance Comparison: Ensembling",
            fill=INK,
    ...
    footnote = (
        ...
        "Pairs: exp010 vs exp014, exp023 vs exp025"
    )
    parts.append(
        svg_text(
            72,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="notebooks/figures/macro_f1_ensemble_comparison.svg",
