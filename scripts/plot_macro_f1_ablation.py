#!/usr/bin/env python3
"""
Render a compact SVG comparing tuned macro-F1 across a few clean A/B studies.

The figure focuses on three experiment pairs:
1. Fixed 5 epochs vs 10 epochs + early stopping patience=2
2. No label smoothing vs label smoothing=0.1
3. No DAPT vs DAPT

Values are read from each experiment's metrics_tuned.json to keep the figure
grounded in the repo outputs instead of duplicated by hand.
"""
...
COMPARISONS = [
    Comparison(
        ...
        reference_label="Fixed 5 epochs (exp004_bertimbau)",
        reference_experiment="exp004_bertimbau",
        variant_label="10 epochs + ES p=2 (exp010_bertimbau_es)",
        variant_experiment="exp010_bertimbau_es",
    ),
    Comparison(
        title="Label smoothing",
        reference_label="No label smoothing (exp010_bertimbau_es)",
        reference_experiment="exp010_bertimbau_es",
        variant_label="Label smoothing 0.1 (exp011_bertimbau_ls)",
        variant_experiment="exp011_bertimbau_ls",
    ),
    Comparison(
        title="Domain-adaptive pretraining",
        reference_label="No DAPT (exp010_bertimbau_es)",
        reference_experiment="exp010_bertimbau_es",
        variant_label="DAPT (exp020_bertimbau_dapt)",
        variant_experiment="exp020_bertimbau_dapt",
    ),
]
...
def render_svg(repo_root: Path) -> str:
    ...
    parts = [
        ...
        '<title id="title">Performance comparison across training experiments</title>',
        (
            '<desc id="subtitle">'
    ...
    parts.append(
        svg_text(
            72,
            90,
            "Performance Comparison Across Training Experiments",
            fill=INK,
    ...
    footnote = (
        ...
        "Pairs: exp004 vs exp010, exp010 vs exp011, exp010 vs exp020"
    )
    parts.append(
        svg_text(
            72,
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        default="notebooks/figures/macro_f1_ablation_comparison.svg",
